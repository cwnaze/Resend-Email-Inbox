...
- **`package.json` pins `overrides: { "html-encoding-sniffer": "5.0.0" }` and that override is load-bearing on prod.** `isomorphic-dompurify` → `jsdom@29` → `html-encoding-sniffer@6`, which is CJS that `require()`s the ESM-only `@exodus/bytes`. Vercel's serverless loader (`/opt/rust/nodejs.js`, with bytecode caching) does not implement `require(ESM)`, so the first module to touch DOMPurify throws `ERR_REQUIRE_ESM` — a 500 on **every** thread load and on the inbound webhook, while local `npm run dev`/`build` pass fine because real Node supports `require(ESM)`. Bumping the adapter runtime to `nodejs24.x` does **not** fix it (tried; Vercel's loader intercepts the require regardless of Node version). `html-encoding-sniffer@5` has the same single-function API jsdom calls and depends on the CJS `whatwg-encoding`. If you ever remove the override, you must verify a thread load on the deployed site, not just locally.
