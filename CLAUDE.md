...
- `@sveltejs/adapter-vercel`'s runtime auto-detection only supports Node 20/22/24. If the local dev machine runs a newer Node (e.g. v26), `npm run build` fails with `Unsupported Node.js version` unless the adapter is given an explicit runtime. This is already set: `adapter({ runtime: 'nodejs24.x' })` in `vite.config.ts`. Don't remove it, and don't drop it back to `nodejs22.x`: `isomorphic-dompurify` → `jsdom@29` → `html-encoding-sniffer@6` is CJS that `require()`s the ESM-only `@exodus/bytes`, which the Vercel Node 22 runtime rejects with `ERR_REQUIRE_ESM` — a 500 on every thread load (and on the inbound webhook). Node 24 supports `require(ESM)`.
