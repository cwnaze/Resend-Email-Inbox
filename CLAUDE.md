...
- [`docs/notes/compose.md`](docs/notes/compose.md) — the pure `lib/compose/addresses.ts` (one definition of "sendable", shared by the Send gate and the action), the recipient field's combobox, the `send` action's validate → send → store order, what Resend does and doesn't do to the threading headers (it rewrites `Message-ID`, preserves `In-Reply-To`/`References` — measured), and reply pre-fill/threading (`?replyTo=<id>` is the only thing the browser chooses; it rides in the form's **action URL** because a form action's query replaces the page's).
