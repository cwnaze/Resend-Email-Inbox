...
- [`docs/notes/compose.md`](docs/notes/compose.md) — the pure `lib/compose/addresses.ts` (one definition of "sendable", shared by the Send gate and the action), the recipient field's combobox, and the `send` action's echo-the-draft shape that US-H02–H05 build on.
...
- **Extend the existing `verify-*.mts` script for an area rather than adding a new ad hoc one**: `db/verify-schema.mts`, `r2/verify.mts`, `auth/verify-auth-sessions.mts`, `webhooks/verify-inbound-webhook.mts`, `inbound/verify-inbound-parse.mts`, `db/verify-inbox-list.mts`, `compose/verify-compose-addresses.mts`. Wrap live-DB work in `try`/`finally` so seeded rows are cleaned up.
