...
- [`docs/notes/contacts.md`](docs/notes/contacts.md) — the `/contacts` read side: `listContacts`' single LEFT JOIN (contacts with no mail must still render, so it is deliberately not the inbox list's INNER join), the address-not-FK correspondence rule over `json_each`, and why US-I02/US-I03's actions have to check the session themselves.
