...
- [`docs/notes/infrastructure.md`](docs/notes/infrastructure.md) — Turso/Drizzle (`db` singleton, `schema.ts`, `db:push`, FK-enforcement and child-before-parent deletes) and Cloudflare R2 (private bucket, presign on demand, server-only imports, and the **hand-applied bucket CORS policy** direct browser uploads depend on).
...
- [`docs/notes/compose.md`](docs/notes/compose.md) — the pure `lib/compose/addresses.ts` (one definition of "sendable", shared by the Send gate and the action), the recipient field's combobox, the `send` action's validate → send → store order, what Resend does and doesn't do to the threading headers (it rewrites `Message-ID`, preserves `In-Reply-To`/`References` — measured), and reply/forward pre-fill and threading (`?replyTo=<id>` / `?forwardOf=<id>` are the only thing the browser chooses; the id rides in the form's **action URL** because a form action's query replaces the page's; a forward clears To, starts a new thread, and copies the original's attachments to new R2 keys). US-H05 adds the picker: the browser uploads **straight to R2** over a presigned PUT (Vercel caps a function's request body ~4.5 MB) and submits only the key, so `server/outbound/uploads.ts` re-derives every size from R2 and accepts only keys it minted — and the bucket needs a **CORS policy** applied by hand (`docs/notes/infrastructure.md`).
- [`docs/notes/contacts.md`](docs/notes/contacts.md) — `/contacts`: `listContacts`' single LEFT JOIN (contacts with no mail must still render, so it is deliberately not the inbox list's INNER join), the address-not-FK correspondence rule over `json_each`, and why every action here checks the session itself. US-I02 adds `updateContactName` — one statement that sets the name **and** clears `auto_created`, because that flag is the only thing stopping the next inbound delivery from overwriting the owner's edit — and the `?edit=<id>` inline form, whose open row is URL state rather than component state.
...
- **Every internal `<a href="/...">` or programmatic navigation must go through `resolve(...)` from `$app/paths`** — `svelte/no-navigation-without-resolve` fails `npm run lint` otherwise. Dynamic routes need the route group in the id: `resolve('/(app)/inbox/[threadId]', { threadId })`. `resolve()` has **no query-string form**; for a path plus a query, build the string in the `<script>` and type it `ResolvedPathname` (from `$app/types`) — the rule accepts that as well as a bare `resolve(...)` call. `ThreadMessage.svelte`'s `replyHref` is the worked example.
- **A form `action="?/name"` replaces the page's whole query string.** Anything the page was parameterized by (`?replyTo=…`) is gone from the URL the action runs on and from the re-render that follows a `fail()`, so a route whose behaviour depends on a search param must put it in the action URL too (`?/send&replyTo=…`). See `docs/notes/compose.md`.
- **Mutating an object _after_ pushing it into a `$state` array does not re-render.** `items.push(obj)` stores a **proxy** of `obj`; a later `obj.status = 'x'` writes straight through to the target and never trips the proxy's setter, so nothing that reads `items` invalidates. Settle an entry by looking it back up (`items.find(i => i.id === id)`) and writing to _that_. Caught in US-H05, where a finished upload sat on "uploading…" forever and Send stayed disabled — `AttachmentField.svelte`'s `settle()` is the worked example. The same trap applies to a reference captured before the push and to anything holding the raw object across an `await`.
...
- `src/lib/server/db/seed-f03-demo.mts` is the shared browser-demo seeder (read thread, unread thread, three-message conversation, three contacts for the compose autocomplete — one of them Cc'd on a seeded message so `/contacts` shows a non-zero count, `123456` login code; `--cleanup` removes it all). Reuse it instead of seeding by hand, and see `docs/notes/inbox-list.md` for how to get a real session cookie in the browser.
