...
- **`package.json` pins `overrides: { "jsdom": "27.0.0" }` and that override is load-bearing on prod — do not drop it or bump it to 28+.** `isomorphic-dompurify` pulls `jsdom`, and jsdom **28+** depends on `@exodus/bytes`, an ESM-only package it `require()`s from CJS (directly in `jsdom/lib/api.js`, and transitively via `html-encoding-sniffer@5`+). Vercel's serverless loader (`/opt/rust/nodejs.js` + `bytecode.js`) does not implement `require(ESM)`, so the first module to touch DOMPurify throws `ERR_REQUIRE_ESM` — a 500 on **every** thread load and on the inbound webhook. jsdom 27 is the newest release with no `@exodus/bytes` anywhere in its tree.
  - **Local `dev`/`build`/`check` cannot catch this**: real Node ≥22.12 supports `require(ESM)`, so everything passes while prod is completely broken. To reproduce Vercel's loader, run with `require(ESM)` disabled: `NODE_OPTIONS=--no-experimental-require-module npx tsx -e "import DOMPurify from 'isomorphic-dompurify'; console.log(DOMPurify.sanitize('<p>x</p>'))"`. That command failing = prod is down. Use it before touching anything in the DOMPurify/jsdom dependency chain.
  - Two things that do **not** fix it, both tried: bumping the adapter runtime to `nodejs24.x` (Vercel's loader intercepts the `require` regardless of Node version), and overriding only `html-encoding-sniffer` (5.0.0 depends on `@exodus/bytes` too, and jsdom 28+ requires it directly anyway).
...
- **Pure modules stay pure** (no env/db/DOM): `inbound/parse.ts`, `inbound/threading.ts`, `lib/inbox/format.ts`, `filter.ts`, `search.ts`, `params.ts`, `srcdoc.ts`. That's what lets the load, the components and the `tsx` verification scripts share one implementation.
- **A module a `tsx` script loads must import by relative path, not `$lib/...`** — bare `tsx` has no Vite alias resolution. Currently constrains `server/db/inbox.ts` and `server/inbox/html.ts`.
- **Extend the existing `verify-*.mts` script for an area rather than adding a new ad hoc one**: `db/verify-schema.mts`, `r2/verify.mts`, `auth/verify-auth-sessions.mts`, `webhooks/verify-inbound-webhook.mts`, `inbound/verify-inbound-parse.mts`, `db/verify-inbox-list.mts`. Wrap live-DB work in `try`/`finally` so seeded rows are cleaned up.
...
- `src/lib/server/db/seed-f03-demo.mts` is the shared browser-demo seeder (read thread, unread thread, three-message conversation, `123456` login code; `--cleanup` removes it all). Reuse it instead of seeding by hand, and see `docs/notes/inbox-list.md` for how to get a real session cookie in the browser.
