...
- **`(app)/+layout.server.ts` protects pages, not endpoints.** A layout `load` never runs for a `+server.ts` request, so any `+server.ts` added under `(app)/` must call `validateSession` itself or it is an unauthenticated hole in the route group. See `docs/notes/auth.md`; `inbox/[threadId]/attachments/[attachmentId]/+server.ts` is the worked example.
